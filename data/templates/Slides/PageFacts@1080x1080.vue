<template>
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1080">
    ...
    <g class="main" clip-path="url(#bounding)">
      ...
      <Dynamic v-for="(fact, index) in factItems" width="800" v-bind:height="fact.height" v-bind:origin="fact.relative.join(' ')">
        <MultiLine v-bind:x="fact.x" v-bind:y="fact.y" v-bind:relative="fact.relative.join(' ')" padding="0" v-bind:text="fact.text" v-bind:align="fact.align" lineheight="1.05" background="none" css="font-size: 60px; font-weight: 300; font-family: 'Jost'; fill: #fff; font-style: italic;"></MultiLine>
      </Dynamic>
  ...
  export default {
    ...
    data() {
      return {
        ...
        facts: [
          {
            text: [
              'LOREM IMPSUM DOLOR SIT AMET ET',
              'DER GANZE LATEIN QUATSCH WUMS',
    ...
    computed: {
      factItems() {
        ...
        const height = 830;
        const margin = 50;
        const intervalHeight = height / (this.facts.length - 1);
        return this.facts.map((fact, index) => {
          const { text } = fact;
          return {
            y: offsetY + (() => {
              if (index == 0) return 0;
              else if (index == this.facts.length - 1) return height;
              else return intervalHeight * index;
            })(),
            x: new Map([
              [0, offsetX],
              [1, 1080 - offsetX]
            ...
            relative: [
              ...
              (() => {
                if (index == 0) return 0;
                else if (index == this.facts.length - 1) return 1;
                else return 0.5;
              })()
            ],
            align: new Map([
            ...
            height: height / this.facts.length - margin,
            text
          }
        });
      }
